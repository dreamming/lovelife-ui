...
export function unMountWall() {
  ...
  const h = mainCanvas.getContext("2d");
  h.clearRect(0, 0, document.body.width, document.body.height);
}
export default function initWall() {
  function w() {
    h.globalCompositeOperation = "source-over";
    h.fillStyle = "rgba(187,120,139," + v + ")";
    h.fillRect(0, 0, f, k);
  ...
  return function() {
    ...
    if (p.getContext) {
      ...
      window.onresize = r;
      r(null);
      const id = setInterval(w, 33);
      // clearInterval(id);
    }
  };
}

export function FSS(container, output) {
  FSS = {
    FRONT: 0,
    BACK: 1,
    DOUBLE: 2,
  ...
  (function() {
    for (
      var b = 0, d = ["ms", "moz", "webkit", "o"], h = 0;
      h < d.length && !window.requestAnimationFrame;
      ++h
    )
      (window.requestAnimationFrame = window[d[h] + "RequestAnimationFrame"]),
        (window.cancelAnimationFrame =
          window[d[h] + "CancelAnimationFrame"] ||
          window[d[h] + "CancelRequestAnimationFrame"]);
    window.requestAnimationFrame ||
      (window.requestAnimationFrame = function(d, h) {
    ...
    for (b = 0; b < this.segments; b++)
      for (d = 0; d < this.slices; d++)
        (g = h[b + 0][d + 0]),
          ...
          (s = h[b + 1][d + 1]),
          this.triangles.push(
            new FSS.Triangle(g, l, n),
            new FSS.Triangle(n, l, s)
          );
  };
        ...
        for (l = b.length - 1; 0 <= l; l--)
          (n = b[l]),
            ...
            this.side === FSS.FRONT
              ? (s = Math.max(s, 0))
              : this.side === FSS.BACK
                ? (s = Math.abs(Math.min(s, 0)))
                : this.side === FSS.DOUBLE && (s = Math.max(Math.abs(s), 0)),
  ...
  FSS.Scene.prototype = {
    add: function(b) {
      b instanceof FSS.Mesh && !~this.meshes.indexOf(b)
        ? this.meshes.push(b)
        : b instanceof FSS.Light &&
          !~this.lights.indexOf(b) &&
          this.lights.push(b);
      return this;
    },
    remove: function(b) {
      b instanceof FSS.Mesh && ~this.meshes.indexOf(b)
        ? this.meshes.splice(this.meshes.indexOf(b), 1)
        : b instanceof FSS.Light &&
          ~this.lights.indexOf(b) &&
          this.lights.splice(this.lights.indexOf(b), 1);
      return this;
    }
  };
  ...
  FSS.CanvasRenderer.prototype.render = function(b) {
    ...
    for (d = b.meshes.length - 1; 0 <= d; d--)
      if (((h = b.meshes[d]), h.visible))
        for (
          h.update(b.lights, !0), g = h.geometry.triangles.length - 1;
          0 <= g;
          g--
        )
      ...
      if (this.program) {
        ...
        if (s || this.vertices !== z)
          for (k in ((this.vertices = z), this.program.attributes)) {
            ...
            for (d = b.meshes.length - 1; 0 <= d; d--)
              for (
                h = b.meshes[d], g = 0, l = h.geometry.triangles.length;
                g < l;
                g++
              )
                for (
                  n = h.geometry.triangles[g], E = 0, y = n.vertices.length;
                  E < y;
                  E++
                ) {
                  vertex = n.vertices[E];
        ...
        for (m in this.program.uniforms)
          switch (((s = this.program.uniforms[m]),
          (d = s.location),
          (b = s.data),
          s.structure)) {
            case "3f":
  ...
  FSS.WebGLRenderer.prototype.buildShader = function(b, d) {
    if (!this.unsupported) {
      ...
      return this.gl.getShaderParameter(h, this.gl.COMPILE_STATUS)
        ? h
        : (console.error(this.gl.getShaderInfoLog(h)), null);
    }
  };
    ...
    for (d = b.meshes.length - 1; 0 <= d; d--)
      if (((h = b.meshes[d]), h.visible))
        for (
          ...
          (l = h.geometry.triangles[g]),
            l.polygon.parentNode !== this.element &&
              this.element.appendChild(l.polygon),
  ...
  FSS.SVGRenderer.prototype.formatPoint = function(b) {
    return (
      this.halfWidth + b.position[0] + "," + (this.halfHeight - b.position[1])
    );
  };
  ...
  /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
    ...
    (function() {
      ...
      function h() {
        ...
        n.autopilot &&
          ((b = Math.sin(n.step[0] * r * n.speed)),
          (d = Math.cos(n.step[1] * r * n.speed)),
          FSS.Vector3.set(y, n.bounds[0] * b, n.bounds[1] * d, n.zOffset));
        for (b = B.lights.length - 1; 0 <= b; b--)
          (d = B.lights[b]),
            FSS.Vector3.setZ(d.position, n.zOffset),
            (k = Math.clamp(
              FSS.Vector3.distanceSquared(d.position, y),
              n.minDistance,
              n.maxDistance
            )),
        ...
        for (m = H.vertices.length - 1; 0 <= m; m--)
          (s = H.vertices[m]),
            ...
            FSS.Vector3.set(
              s.position,
              l.xRange * H.segmentWidth * b,
              l.yRange * H.sliceHeight * d,
              l.zRange * z * k - z
            ),
